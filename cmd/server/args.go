...
func (c *cmdlineArgs) Parse() error {
	...
	flag.DurationVar(&c.shutdownTimeout, "timeout", time.Second*15, "the duration for which the server gracefully wait for existing connections to finish - e.g. 15s or 1m")
	flag.Parse()
